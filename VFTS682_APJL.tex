...
\affil{ $^{*}$ Corresponding author.  A list of the affiliations can be found at the end of this paper.}
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{llc|c|c}
      ...
      \hline
         &$\mathrm{RA}_\mathrm{VFTS682}$&[degrees] & 84.7314 %$ \pm 0.036\,\mathrm{mas} $ 
                     & (1) \\        
      ...
      \hline
          &$\mu_\mathrm{RA}$&[$\masyr$] & $1.843\pm 0.070$
               ...
               &$\mu_\mathrm{DEC}$&[$\masyr$] & $0.786\pm 0.080$
                     &  (1) \\        
                 & $\rho\,(\mu_\mathrm{RA}, \mu_\mathrm{DEC})$ &[$\masyr$] & $0.023$
                        & (1)  \\         
...
\hline
  ...
  %    \multicolumn{5}{l}{\emph{Position}} \\
      ...
      &$\delta\mu_\mathrm{}$  &[$\masyr$] & $0.13\pm0.09$ &  (1,4) \\
                 &$v_\mathrm{2d}$  &[$\kms$] & \SdM{$31\pm 21$} (not $32\pm21$!) & (1,4)\\  
                     &$\theta$  &[degrees] &  \SdM{$14.2_{-xx}^{+xx}$}  & (1,4)\\  
      ...
      &$\delta\mu_\mathrm{DEC, HST}$  &[$\masyr$] & $0.20\pm0.10$ &  (3) \\
       &$\delta\mu_\mathrm{HST}$  &[$\masyr$] & $0.20\pm0.10$ &  (3) \\
                  &$v_\mathrm{2d, HST}$  &[$\kms$] & $47\pm24$ & (3)\\  
                      &$\theta^{*}$  &[degrees] &   \SdM{$xx_{-xx}^{+xx}$}   & (1,4)\\  
                          &$v_\mathrm{2d, \parallel}$  &[$\kms$] & $\SdM{xxx}\pm\SdM{xxx}$ & (1,4)\\  
 %
...
\begin{figure*}[htbp]
  ...
  \includegraphics[width=0.35\textwidth]{figures/angle_dist}
  \caption{Left panel: distribution of OB-type and Wolf-Rayet stars in proper
    ...
    lighter blue histograms contain 36 stars with errors smaller than $0.05\,\mathrm{mas \
      yr^{-1}}$. \SdM{ Todo: [1] insert correct errorbars in right pabel, [2] make (a) linewidth, (b) line width of the hashed shading and (c)  figure height consistent. (You could try labeling the top x axis in the right panel in degrees.)  [3] fix normalization to be consistent in both pabels, [4] unit in axis label in righthand plot is not correct. Degrees but radian. [4] Legen: the rel error limits that we adopted are missing a unit. Milia arrsecond?    }}
  \label{fig:mu_dist}
\end{figure*}
...
%\footnote{\url{https://vizier.u-strasbg.fr/viz-bin/VizieR-3?-source=I/345/gaia2}}
...
\noindent $^{1}${Astronomical Institute Anton Pannekoek, University of
 ...
 \SdM{$^{x}$ {Affiliation: Chris Evans}\\
  $^{x}$ {Affiliation: Paco Najorro}\\}
