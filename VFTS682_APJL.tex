...
\title{Space astrometry of the very massive $\sim$$150\,M_\odot$
  candidate runaway star VFTS682}
...
cluster R136. % Its apparent isolation
The absence of other massive stars in its immediate vicinity
led to two intriguing hypotheses: % posed in earlier work:
either it formed in relative isolation % through a new mode of star formation
or it was ejected dynamically from the central cluster. 
 
% Aiming to shine light on this debate,
In this context, 
we investigate the kinematics of
VFTS682 as obtained by \emph{Gaia} and multi-epoch \emph{Hubble Space
  Telescope} astrometry. We derive a projected velocity relative to
the cluster of $38\pm17\kms$ ($1\sigma$ confidence interval).  The direction and magnitude of
the proper motion are consistent with VFTS682 being a slow runaway
ejected from the central cluster, given its distance and the inferred
age. The \todo{rephrase positive, see Fabian and Roeland's comments}



However, the error bars are substantial and the proper motion measurements alone cannot rule out the counter hypothesis with high confidence.   
 
If future data confirm the runaway nature, this would make VFTS682
the most massive runaway star known to date. This hypothesis is
further supported by a variety of
...
The very massive O stars VFTS16 (and possibly VFTS72), which has recently been proposed as a dynamically ejected runaway, provides direct evidence that R136 is indeed capable of ejecting some of its most massive members, consistent with the predictions of numerical simulations.
\end{abstract}
...
\citep{lada:03}, where massive stars reside in the
innermost cores. In this picture, field stars are primarily the result
...
formed in the field \citep[e.g.,][]{parker:07}. Another, not
...
the recent findings of proper motion studies \citep[][]{lennon:18, drew:18}.
...
the most luminous stars in the region. The absence of radial velocity
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{llc|c|c}
      ...
      \hline
         &$\mathrm{RA}_\mathrm{VFTS682}$&[degrees] & \phantom{-0}84.7314 %$ \pm 0.036\,\mathrm{mas} $ 
                     & (1) \\        
               &$\mathrm{DEC}_\mathrm{VFTS682}$&[degrees] & \phantom{0}-69.0741%$ \pm 0.048\,\mathrm{mas} $
                     & (1)  \\    
               ...
               &$     \mathrm{DEC}_\mathrm{R136}$&[degrees] &  \phantom{0}-69.1009
                     &  SIMBAD \\       
        &$      \delta\mathrm{RA}$  &[mas] & \phantom{-0}0.0547                      
        &  (2, 5)
  \\        
               &$     \delta\mathrm{DEC}$  &[mas] & \phantom{-0}0.0268 
                     &  (2, 5) \\  
                       &$  d_\parallel$  & [arcsec]
                         & 119.4
                                 &  SIMBAD \\
...
for over-interpreting the estimates for the radial velocities. % , but we will summarize the estimates that have been published. 
...
data give different estimates for the RV ({\color{blue} Rubio-D{\'
    i}ez et al. in prep}). \cite{bressert:12} note an offset compared between the RV of
...
\vspace*{10pt}% to move new sec. to newpage
\begin{figure*}[tbp]
  \centering
...
\begin{figure*}%[tbp]
  \centering
  \includegraphics[width=\textwidth]{./figures/Figure2_paper_vfts682_hst_corr_nor136err}  
...
prevents this issue. % should be one of the brightest
% stars in the region, however, the high extinction makes it faint enough such that saturation effects are not an issue.
VFTS682 is relatively slow and is therefore not discussed in their paper.  However, their measurements are useful here, since they provide an  estimate of the proper motion of VFTS682 that is independent from the \emph{Gaia} measurement. 
...
to R136 of
VFTS682. For comparison, we also show the relative
...
blue). $\theta$ is the angle between the projected radial direction from the
...
% it were ejected from R136 early in its lifetime. We highlight this star together with the two O-type
% runaways, VFTS16 and VFTS72, \mbox{(re-)identified} by \cite{lennon:18}.

% The left panel shows the distribution in proper motion relative
% to R136 from \emph{Gaia} DR2, selecting stars with small error bars only.
% Although VFTS682 is not peculiar, its relative proper motion
% falls in the range expected for it to reach its present day isolation
% within its apparent age (green vertical band). This suggests it is indeed a ``slow-runaway'' as
% suggested by \cite{bestenlehner:11}.
Subtracting
...
% % it
% is consistent with dynamical ejection from R136.
We note that
...
from the disruption of a binary is excluded. %  since the region is too young for stars
...
% \cite{dekoter:97,crowther:10,crowther:16}
 with a remarkable similarity in particular with
...
R136. Such a binary could be R145 according to \cite{fujii:11}, and it might be an
