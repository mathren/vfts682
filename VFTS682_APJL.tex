...
% Observations with the Chandra X-Ray Observatory do not reveal an X-ray point source at the position of VFTS682 \cite{townsley:06}. Such a detection would be expected if the star hosted an accreting compact object or if the strong winds of VFTS682 are colliding with those of a stellar companion. This star is also relatively isolated in \emph{Spitzer} images \citep{meixner:06}, except for the extended source S10 interpreted as star formation triggered by the wind of VFTS682 itself by \cite{walborn:13}. 
% >>>>>>> df97f49fb6e094f5c7a3bd15402c437202858eaa
...
\SdM{VFTS682 also appears to be relatively isolated in (near) infrared.   There nearest bright (near) infrared sources detected by \emph{Spitzer}  \citep{meixner:06} and resolved in the \emph{VISTA} Magellanic Clouds Survey \citep{cioni:11} are located at a distance of about 10 arcsecond, i.e. about 2.4 pc. \cite{walborn:13} speculate that these nearby young stars may represent a case of starformation triggered by the the wind of VFTS682, but we chance alignment cannot be excluded.  }

% interpreted as star formation triggered by the wind of  VFTS682 itself by \cite{walborn:13}.   - SdM: too strong lets be more careful.  This is a controversial topic.  Walborn formulates more careful than you summarize him. 
...
\citep[see][]{lennon:18}. The thick cyan arrows are proportional to
the relative proper motion from \emph{Gaia} DR2, and the yellow thick
...
\begin{figure}%[tbp]
  ...
  \caption{The thick cyan arrows indicate proper motions relative R136 (blue
    circle) from \emph{Gaia} DR2, multiplied by 1\,Myr. The prolongations in the opposite
    direction are proportional to the apparent ages of the stars, and
    the thin cyan lines illustrate the error cone. The yellow arrow
    indicates the HST proper motion of VFTS682, with the relative
    error cone and prolongation proportional to its apparent age.
    }
