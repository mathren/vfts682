...
\author{M.~Renzo$^{*,1}$, S.~E.~de~Mink$^{1}$, D.~J.~Lennon$^{2}$, \SdM{I.~Platais$^{3}$,}
  R.~P.~van~der~Marel$^{3,4}$,  \SdM{E.~L.~Laplace$^{1}$,}
...
connecting the center of R136 with the current position of VFTS 682.  This means that $\theta = 0$ is the angle that is expected for a star that is moving away from R136 with a perfect radial motion.   All kinematic quantities are provided in
Tab.~\ref{tab:vfts682}. 
...
the uncertainty in the present-day age of the star (cf.~\Tabref{tab:star}).
...
(HST). The lighter arrows show the corresponding opening angle within
the large uncertainties. The length of each arrow is proportional to
...
Assuming VFTS682 originates from R136, we can calculate its kinematic
age simply as:
\begin{equation}
  \label{eq:kin_age}
...
(slow) runaway known to date, with a peculiar three-dimensional speed of
$44\pm21\,\kms$ \todo{make 2d? use LOS velocity from Marimar?}. Due to the large error bars, this result will need
