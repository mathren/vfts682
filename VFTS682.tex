...
\author{M.~Renzo$^{1}$, S.~E.~de~Mink$^{1}$, D.~J.~Lennon$^{2}$,
  R.~P.~van~der~Marel$^{3,4}$, J. Bestenlehner$^{5}$, V.~H\'enault-Brunet$^{6}$, S.~Justham$^{7,8}$, F.~R.~Schneider$^9$, J.~Vink$^{10}$}
  
  % ,
%  A.~de~Koter$^{1}$ \todo{... add coauthors...} }

\affil{$^{}$Astronomical Institute Anton Pannekoek, University of
    Amsterdam, 1098 XH Amsterdam, The Netherlands \\
    $^{}$ ESA, European Space Astronomy Centre, Apdo. de Correos 78,
...
%
Previous spectroscopic studies identified VFTS 682 as one of the most massive stars known. It resides in the field of the 30 Dor region in the Large Magellanic Cloud at a projected distance of 29\,pc of the star cluster R136. The lack of a detected offset in radial velocity derived from the spectra, led to speculations whether such extreme stars can form in relative isolation. 
%
...
projected two-dimensional speed of about $\sim$$34\pm1 \kms$,
implying a three-dimensional speed of about $\sim$$40\pm20\kms$ when
...
%% using
%% /scratch/mathieu/MMPS/SCRIPTS_dirty/my_script/binaries/binary_c/30Dor/vfts682/data/asu.tsv to find average
...
\tableline
  \end{tabular}
  \tablecomments{We truncate the decimals on RA and DEC and the
    relative errors for the purpose
    of display. Readers are encouraged to consult the \emph{Gaia} DR2 catalog
  for accurate values.}
    \end{center}
  \label{tab:vfts682}
\end{table}
...
GO-12499 with both cameras, \citealt{platais:15}) to determine the
proper motion of massive stars in the 30 Doradus region. For
large distances ($\gtrsim50$\,kpc), HST data can compete or even
exceed Gaia data in quality.

In the catalog from \citet{platais:18} they listed the proper motion
components of VFTS682 relative to the field, providing a
measure of the motion of this star completely independent from
ours. HST images of VFTS682 are not saturated in any epoch \todo{?},
thanks to the large extinction toward this star. They found proper
...
% %% pure HST physical velocities
% \todo{done in the train: to be checked} Converting these into physical velocities we obtain 
% $\delta v_\mathrm{RA}=2.3\pm30\,\kms$, $\delta v_\mathrm{DEC}=47\pm\,23\kms$, corresponding to
% $v_\parallel^\mathrm{HST} = 47\pm\,1\kms$.
