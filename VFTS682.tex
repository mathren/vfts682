...
likely conclusion looking at the general region: it is a ``bona
  fide'' runaway star with velocity components $\left(v_\mathrm{RA}, v_\mathrm{DE}, v_\mathrm{rad}\right)$ = $\left(\todo{XX},\todo{YY},\todo{ZZ}\right)$, corresponding to $v\equiv\sqrt{v_\mathrm{RA}^2+v_\mathrm{DE}^2+v_\mathrm{rad}^2}=\todo{AA}\,\kms$.
