...
\author{M.~Renzo$^{1}$, S.~E.~de~Mink$^{1}$, D.~J.~Lennon$^{2}$,
  R.~P.~van~der~Marel$^{3,4}$, I.~Platais$^{5}$, J. Bestenlehner$^{6}$,
  A.~de~Koter$^{1}$, V.~H\'enault-Brunet$^{7}$, S.~Justham$^{8}$, H.~Sana$^{9}$, F.~R.~N.~Schneider$^{10}$, J.~S.~Vink$^{11}$}
  
\affil{\todo{check affil. num}$^{}$Astronomical Institute Anton Pannekoek, University of
...
\date{}
\begin{abstract}
The formation and fates of the most massive stars remains among the most elusive questions in stellar astrophysics. 
%
...
star cluster Radcliffe 136 (R136). The lack of a detected offset in radial velocity\todo{can we really say that?RV
  is about 30kms...}
derived from the spectra, led to speculations whether such extreme
stars can form in relative isolation . 
%
\todo{add mention of HST}We use the unprecedented proper motions from the second \emph{Gaia} data
...
projected two-dimensional speed of about $\sim$$34\pm17\kms$ ,
...
\Tabref{tab:vfts682} for the coordinates from \emph{Gaia} DR2 in the
ICGS frame]{evans:11}
...
wind obscuring the stellar surface. 
...
% because of the large extinction toward this star.
...
\begin{center}
  ...
  \tablecomments{We do not list the error on the RA and DEC positions,
    ...
    less sensitive to the wind velocity structure.}
    \end{center}
  \label{tab:vfts682}
\end{table}
