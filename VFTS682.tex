...
\title{Gaia identifies VFTS682 as the most massive, dynamically ejected runaway star known to date}

\author{M.~Renzo$^{1}$, S.~E.~de~Mink$^{1}$, D.~J.~Lennon$^{2}$,
  S.~Justham$^{3,4}$, J.~Vink$^{5}$, A.~de~Koter$^{1}$ \todo{... add coauthors...} }
...
this population still remains debated \citep{lamb:16}.   One
...
\cite{bestenlehner:11}, who discuss the case of the very massive star
...
$150.0^{+28.7}_{-17.4}\,M_\odot$ and a mass loss rate of
$\sim10^{-4.1\pm0.2}\,M_\odot \ \mathrm{yr}^{-1}$
\citep[][]{bestenlehner:11,schneider:18}, which make it one of the
most extreme objects in the region.  It is reminiscent of the very
massive stars, a.k.a. ``monster stars'', identified by
...
of $150.0^{+28.7}_{-17.4}\Msun$, using the Bayesian analysis tool Bonsai
\citep{schneider:17}, based on the evolutionary models from
\citet{brott:11, kohler:15}. These values place VFTS682 around the ``canonical upper limit'' of $\sim$$150\Msun$  by \citet{figer:05}.
...
as predicted from numerical N-body simulations by \cite{fujii:11, banerjee:12}.
