...
\author{M.~Renzo$^{1}$, S.~E.~de~Mink$^{1}$, D.~J.~Lennon$^{2}$,
  R.~P.~van~der~Marel$^{3,4}$, J. Bestenlehner$^{5}$,
  A.~de~Koter$^{1}$, V.~H\'enault-Brunet$^{6}$, S.~Justham$^{7,8}$,
  I.~Platais$^{9}$, H.~Sana$^{}$, F.~R.~Schneider$^{10}$, J.~S.~Vink$^{11}$}
...
projected two-dimensional speed of about $\sim$$34\pm1 \kms$ \todo{fix
  error!},
...
\tableline
  \end{tabular}
  \tablecomments{\todo{the error on position is in milliarcsec, not
      degrees! Fix this, Roeland is right! Also, report relative pm,
      not absolute}}
    \end{center}
  \label{tab:vfts682}
\end{table}
